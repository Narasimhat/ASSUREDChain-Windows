...
if fixed_snapshot_file.exists():
    try:
        existing_snapshot_data = json.loads(fixed_snapshot_file.read_text())
    except Exception:
        pass
...
def save_section_data(section_name: str, section_data: dict):
    ...
    try:
        ...
        if fixed_snapshot_file.exists():
            full_data = json.loads(fixed_snapshot_file.read_text())
        else:
            full_data = {
        ...
        st.caption(f"Saved to: {fixed_snapshot_file}")
        return True
...
if submitted:
    ...
    if not fixed_snapshot_file.exists():
