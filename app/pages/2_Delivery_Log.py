...
if submitted:
    try:
        ...
        outfile = ensure_local_path(snapshot_dir / f"{log.project_id}_{log.timestamp_unix}_{digest[:12]}.json")
...
if snapshot:
    ...
    if st.button("Anchor delivery hash on-chain (Sepolia)", disabled=anchor_disabled):
        try:
            ...
            proof_path = ensure_local_path(Path(snapshot["outfile"]).with_suffix(".chainproof.json"))
